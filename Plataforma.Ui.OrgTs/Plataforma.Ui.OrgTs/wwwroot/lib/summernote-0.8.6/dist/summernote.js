...
(function (factory) {
  ...
  */
  ...
  var Table = function () {
     ...
     * @return {Node}
     */
      this.cleanColor = function (rng) {
          var cell = dom.ancestor(rng.commonAncestor(), dom.isCell);
          var currentTable = $(cell).closest('table');
          currentTable.removeClass('table-bordered').addClass('table-dashed');
          currentTable.find('td').addClass('table-dashed');
          currentTable.find('th').addClass('table-dashed');
      };
  };
  var KEY_BOGUS = 'bogus';
   ...
   *
   */
    this.cleanColor = function () {
        var rng = this.createRange($editable);
        if (rng.isCollapsed() && rng.isOnCell()) {
            beforeCommand();
            table.cleanColor(rng);
            afterCommand();
        }
    };
  ...
  var Buttons = function (context) {
    ...
    this.addTablePopoverButtons = function () {
      ...
      context.memo('button.deleteTable', function () {
        ...
        /// --> feito por mim
      context.memo('button.cleanColor', function () {
          return ui.button({
              className: 'btn-md',
              contents: ui.icon(options.icons.cleanColor),
              tooltip: lang.table.cleanColor,
              click: context.createInvokeHandler('editor.cleanColor')
          }).render();
      });
        ////-----------------
    };
  ...
  $.summernote = $.extend($.summernote, {
    ...
    options: {
      ...
      popover: {
        ...
        table: [
          ['add', ['addRowDown', 'addRowUp', 'addColLeft', 'addColRight']],
          ['delete', ['deleteRow', 'deleteCol', 'deleteTable', 'cleanColor']]
        ],
        air: [
          ['color', ['color']],
          ['font', ['bold', 'underline', 'clear']],
